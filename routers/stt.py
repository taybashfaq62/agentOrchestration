...
def _run_silero_vad(audio_np: np.ndarray, sr: int = 16000, threshold: float = 0.5) -> np.ndarray:
    ...
    Returns original audio unchanged if no speech segments found.
    """
...
async def transcribe(
    ...
    - faster_whisper: better for long audio, multilingual, any sample rate.
                      Always uses built-in VAD (vad_filter=True).
    - moonshine:      better for short clips (<30s), English only, very low latency.
                      Pass vad=true to pre-filter silence with Silero VAD.
    """
